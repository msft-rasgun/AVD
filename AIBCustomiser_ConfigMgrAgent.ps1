...
Write-Log "Start script AIBCustomiser_ConfigMgrAgent v0.1"

If (Test-Path C:\Windows\Temp\ccmsetup.exe){
    Start-Process -FilePath C:\Windows\Temp\ccmsetup.exe -ArgumentList @("/mp:TEST-ADHA01.gundel.local", "SMSMP=TEST-ADHA01.gundel.local", "SMSSITECODE=DH1", "PROVISIONTS=DH120001")
}
else {
    Write-Log "C:\Windows\Temp\ccmsetup.exe not found" -Err
    exit 1
}
