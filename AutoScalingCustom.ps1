...
<#
.SYNOPSIS
    Runbook script to perform up- and down-scaling operations in Azure Virtual Desktop
...
.DESCRIPTION
    ...
    This custom script have been modified to include the following functionality
        Scale up during off-peak
        Scale down during peak
        Definition of separate minimum number of VMs in peak and off-peak
        Added calculation to minimum number of VMs to include a minimum number of cores to ensure we don't exceed threashold when scaling down
        Weekend definition and option to treat weekend days as 100% off-peak

    This runbook may require modifications to the default logic app to provide the correct inputs
...
.INPUTS
    ...
    Required
        ResourceGroupName               [Resource group name where the hostpool resides]
		...
		BeginPeakTime                   [Format hh:mm, start of peak]
		EndPeakTime                     [Format hh:mm, end of peak. If earlier than BegingPeakTime, it assumed it's ending the next day]
        LimitSecondsToForceLogOffUser   [Time in seconds before forcing users to logoff as part of scaling down. Disabled if set to 0]
        LogOffMessageTitle              [Text in title bar of the user logoff notification. Only required if LimitSecondsToForceLogOffUser is set to a non-zero value]
        LogOffMessageBody               [Body of the user logoff notification. Only required if LimitSecondsToForceLogOffUser is set to a non-zero value]
        SessionThresholdPerCPU          [Threshold value of user sessions per core]
        MinimumNumberOfRDSH             [Minimum number of session hosts to keep running in off-peak]
    ...
    Optional
	    ...
	    MaintenanceTagName              [Session hosts with this maintenance tag will be excluded from the scaling script]
        WeekendOffPeak                  [Determines if the weekend should be regarded as 100% off-peak. By default, this is set to false]
        Weekdays                        [Specify which days are part of the working week. Default values are @("Monday","Tuesday","Wednesday","Thursday","Friday")]
        PeakMinRunningVMs               [Minimum number of session hosts to keep running in peak. If not set, it will default to the value of MinimumNumberOfRDSH]
        ...
        SkipUpdateLoadBalancerType      [The script will not change the load balancer type of the hostpool to BreathFirst]
        StatusCheckTimeOut              [Specify time in seconds to wait for jobs to finish. Default value 3600 (1hr)]

.NOTES
    v1.0.0 Initial version - 2021-09-29
#>
...
try {
    ...
    [bool]$WeekendOffPeak = get-PSObjectPropVal -Obj $RqtParams -Key 'WeekendOffPeak' -Default $false
    [array]$Weekdays = get-PSObjectPropVal -Obj $RqtParams -Key 'Weekdays' -Default @("Monday","Tuesday","Wednesday","Thursday","Friday")
	[int]$PeakMinRunningVMs = get-PSObjectPropVal -Obj $RqtParams -Key PeakMinimumNumberOfRDSH -Default $MinRunningVMs
	...
	function Set-nVMsToStartOrStop {
		...
		# check if need to adjust min num of running session hosts required if the number of user sessions is close to the configured per core threshold
        If ($MinRunningVMs -and $nRunningVMs){
            [double]$MinRunningCores = $nUserSessions / $UserSessionThresholdPerCore
            [double]$CorePerVM = $nRunningCores / $nRunningVMs
            if ($MinRunningCores / $CorePerVM -gt $MinRunningVMs) {
                $MinRunningVMs = [math]::Ceiling($MinRunningCores / $CorePerVM)
                Write-Log "Number of user sessions exceeds the user per core threshold with the current minimum number of session hosts. Adjusted minimum number of running session hosts required to $MinRunningVMs"
            }
        }
    
        # Check if we need to start VMs as part of peak hours
        If ($InPeakHours){
            If ($PeakMinRunningVMs -gt $MinRunningVMs){
                Write-Log "[In peak hours] Adjusted minimum number of running session hosts required: $PeakMinRunningVMs"
                $MinRunningVMs = $PeakMinRunningVMs
            }
        }
        ...
        # Check if the current load is higher than the threshold
        ...
        if ($nUserSessionsPerCore -gt $UserSessionThresholdPerCore) {
            ...
            Write-Log "Number of user sessions per core is more than the threshold. Need to start $($res.nCoresToStart) cores"
        }

        If ($res.nCoresToStart + $res.nVMsToStart -gt 0){
            Write-Log "Scaling up, no need to check if we need to scale down"
        }
        else {
            if ($nRunningVMs -gt $MinRunningVMs) {
                # Calculate the number of session hosts to stop
                $res.nVMsToStop = $nRunningVMs - $MinRunningVMs
                Write-Log "Number of running session host is greater than minimum required. Try to stop $($res.nVMsToStop) VMs"
            }                
        }
	}
    ...
    If ($WeekendOffPeak){
        Write-Log 'Configured to treat weekend as off peak'

        # Check if day of the week defined as a Weekday
        If ($Weekdays -contains $CurrentDateTime.DayOfWeek){
            Write-Log "Current day [$($CurrentDateTime.DayOfWeek)] is a week day, use normal peak calculation"
            ...
            Write-Log "Current day [$($CurrentDateTime.DayOfWeek)] is a weekend day, set to off peak"
            [bool]$InPeakHours = $false
        }
    }
    else {
        Write-Log 'Weekend not excluded from peak time calculation'
        [bool]$InPeakHours = ($BeginPeakDateTime -le $CurrentDateTime -and $CurrentDateTime -le $EndPeakDateTime)
        if ($InPeakHours) {
            Write-Log 'In peak hours'
        }
